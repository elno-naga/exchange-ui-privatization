...
const getSetData = async (domainData, host, app, fileBasePath, api, lan, cusSkin) => {
  ...
  if (domainData) {
    ...
    const nowHost = app.ctx.request.header.host;
    const res = await app.ctx.curl(`${urlHost}${apiProxy}/${api}`, {
      ...
      data: JSON.stringify({
        ...
        domain: api.includes('public_info_v4') ? nowHost : undefined,
      }),
    });
    if (res.status === 200 && res.data.code.toString() === '0') {
      if (api.indexOf('public_info') > -1) {
        if (res.data.data.skin_new) {
          const skin = await setSkin(domainData, app, app.config.skinsPath, res.data.data.skin_new);
          if (skin) {
            res.data.data.skin = skin;
          }
        }
