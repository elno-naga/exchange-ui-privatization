  ...
  class MarketExtend {
    ...
    initScroll() {
      if (window.IScroll) {
        ...
        this.$homeTbody.querySelector('.iScrollIndicator').style.background = colorMap['fill-5-bg'];
      } else {
    ...
    createMarketTitle(dataList, coinList) {
      let html = '';
      html += `<li class="main-1-bd" data-market="myMarket">
      ...
      dataList.forEach((data, index) => {
        ...
        data.data.forEach((item) => {
          ...
          let evenSymbolHtml = `<div class="even">
                                    ...
                                    <span class="evenSymbol" data-id="${item.id}" data-eftOpen="${item.etfOpen}">${item.data[0][1].iconSvg}</span>
                                            ${item.etfOpen && this.marketCurrent !== 'ETF' ? '<span class="eft-class main-1-bd main-1-cl">ETF</span>' : ''}
                                </div>`;
          if (this.lxg) {
            let img = '';
          ...
          childHtml += `<li class="home-tbody-li ${liBorder} ${liColor} ${this.tableBg}" data-coin="${evenSymbol}" data-realCoin="${item.id}">
                                        ...
                                        <div class="even newPrice item1">
                                            ...
                                            <p class="text-2-cl subPrice">${item.data[1][0].subContent.text}</p>
                                        </div>
                                        <!-- 涨跌幅 -->
        ...
        if (dataLen > 1) {
          if (this.lxg) {
            ...
            html += `<div class="home-tbody" data-coin="${data.title}">
                                 <div>
                                     <div class="home-tbody-title fill-6-bd ${this.titleBg ? this.titleBg : ''}">
                                         <span class="lable fill-6-bd">${data.title}</span>
                                     </div>
                                     <ul class="home-tbody-list">${childHtml}</ul>
                                 </div>
                             </div>`;
