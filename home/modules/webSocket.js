(() => {
  ...
  const init = () => {
        ...
        if (itemData) {
          ...
          if (coinLabel && Number(window.coinTagOpen)) {
            const str = `<div class="coin-label">
              <span class="coin-text main-1-cl">${coinLabel}</span>
    ...
    // 首页市场推荐数据显示全部类型币对
    const setRecMarketData = (coinList, symbolAll, initData) => {
      if (!marketDataObj) return;
      const marketDataList = [];
      const keyarr = Object.keys(marketDataObj);
      keyarr.forEach((item) => {
        const itemData = marketDataObj[item];
        if (itemData) {
          const showName = getCoinShowName(itemData.name, symbolAll);
      ...
      emitter.emit('MARKET-REC-DATA', marketDataList.sort((a, b) => a.sort - b.sort), initData);
    };
