(() => {
  ...
  const { getCookie } = window.BlockChainUtils;
  ...
  const formatHeaders = (acHeaders) => {
    ...
    if (window.env === 'local'){
      headers.device = '1b66b402acedc88f2b1495bbff740be4';
      // headers['exchange-token'] = 'b0d758f848b79b9606c09f69142b77f1234e6a58102b4c66883a0658a30c7b33';
    }
    if (acHeaders) {
      Object.keys(acHeaders).forEach((item) => {
        headers[item] = acHeaders[item];
      });
    }
    return headers;
  };
  ...
  const request = function request(
    url, headers,
    params, method, hostType, resolve, reject,
  ) {
    return fetch(url, {
      headers: formatHeaders(headers),
      body: method === 'GET' ? undefined : JSON.stringify(formatParams(params)),
      method: method || 'post',
    }).then((res) => res.json()).then((data) => {
      if (data.code) {
        if (data.code.toString() === '10002') {
          window.emitter.emit('outUserIsLogin');
        }
      }
      resolve(data);
    }).catch((err) => {
  ...
  const fetchData = ({
    ...
    if (!window.secur) {
      ...
      window.security.init(msec, (info) => {
        window.secur = info;
        request(url, headers, params, method, hostType, resolve, reject);
      });
    } else {
      request(url, headers, params, method, hostType, resolve, reject);
    }
