...
import {
  setCoMarket, myStorage, getDigit, getCookie,
} from '@/utils';
...
export default {
  ...
  mutations: {
    ...
    ACTIVE_CONTRACT_DATA(state, data) {
      ...
      // 最小下单额精度
      state.minOrderMoneyFix = getDigit(data.coinResultVo.minOrderMoney);
      // 合约币对价格精度
    ...
    CONTEACT_PROINFO(state, data) {
      // 用户配置信息
      state.contractProInfo = data;
    },
    // 前台公共实时信息
    ...
    POSITION_LIST_PRO(state, data) {
      state.positionListPro = data;
    },
    // 币种列表
    ...
    // 真实保证金币种列表
    ORIGINAL_SYMBOL_LOST(state, data) {
      state.originalCoinList = data;
    },
    // 可平数量列表
    ...
    // 当前合约历史订单列表
    CURRENT_HISTORY_OTDER(state, data) {
      state.currentHistoryOrderLis = data;
    },
    // 当前普通单委托数量
    ...
    // 标记价格列表
    TAG_PEICE_LIST(state, data) {
      state.tagPriceList = data;
    },
    // 资产数据 钱包余额 总权益 未实现盈亏
    AMOUNT_INFO_DATA(state, data) {
      state.amountInfoData = data;
    },
    // 兑换法币汇率
    SAVE_RATE_DATA(state, data) {
      state.rateData = data;
    }
  },
  actions: {
    ...
    getFutorePublicInfo({ commit, dispatch }) {
      axios({
        ...
        if (rs.code === "0") {
          const { contractList, marginCoinList, originalCoinList } = rs.data;
          ...
          if (rs.data.contractProInfo) {
            commit("CONTEACT_PROINFO", rs.data.contractProInfo);
          }
          ...
          if (contractList && contractList.length) {
            ...
            contractList.forEach(item => {
              ...
              // 最小下单额精度
              contractListMap[contractName].minOrderMoneyFix = getDigit(dataItem.coinResultVo.minOrderMoney);
              // 抽取保证金币种信息
              marginCoinInfor[dataItem.marginCoin] = {
                // 真实保证金币种
                originalCoin: dataItem.originalCoin,
                // 保证金币种
            ...
            // 真实保证金币种列表
            commit("ORIGINAL_SYMBOL_LOST", originalCoinList);
            // 合约列表信息 MAP
    ...
    setActivePublicInfo({ commit, state, dispatch }) {
      ...
      contractList.forEach(item => {
        if (contractName === item.contractName) {
          ...
          // 当前合约币对 小写加类型（WS的时候需要用到）
          if (item.subSymbol) {
            commit("CONTRACT_TYPE_SYMBOL", item.subSymbol);
          } else {
    ...
    // 获取持仓列表+资产
    getPositionList({ commit }, type) {
      let urls = this.state.url.futures.getAssetsList;
      if (type) {
        urls = this.state.url.futures.closeOrOpenPosition;
      }
      axios({
        url: urls,
        method: "post",
    ...
    getUserOrderCount({ commit, state }) {
      axios({
        ...
        params: {
          // contractId: state.contractId,
          orderContractId: state.contractId, // myStorage.get('isShowCurPosition1') ? state.contractId : null,//普通委托合约ID
          triggerContractId: state.contractId,//  myStorage.get('isShowCurPosition2') ? state.contractId : null,//条件委托合约ID
        }
      }).then(rs => {
        if (rs.code === "0") {
    ...
    // 获取用户委托计数
    getPriceList({ commit, state }, coin) {
      if (!coin) {
        return false
      }
      let priceBasis = 0
      if (state.futureUserConfig) {
        priceBasis = state.futureUserConfig.priceBasis;
      }
      axios({
        // url: 'common/price_list',
        url: 'position/wallet_and_unrealized',
        method: 'post',
        hostType: 'co',
        params: {
          // contractId: state.contractId,
        ...
        if (rs.code === '0') {
          const {walletAmount, unRealizedAmount, equity, positionVos} = rs.data || {}
          commit('AMOUNT_INFO_DATA', {
            walletAmount,
            unRealizedAmount,
            equity,
          });
          if (positionVos && positionVos.length) {
            let newObj = {}
            positionVos.forEach(item => {
              Object.assign(newObj, item);
            })
            commit('TAG_PEICE_LIST', newObj);
          }
        }
      });
    },
    // 获取法币汇率 现货接口（ex）
    getPratev2({ commit, state }) {
      ...
      axios({
        ...
        if (rs.code === '0') {
          if (rs.data && rs.data.rate) {
            commit('SAVE_RATE_DATA', rs.data.rate);
          }
        }
      });
    },
