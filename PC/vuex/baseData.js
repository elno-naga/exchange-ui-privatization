...
const UPDATE_SAFE_WITHDRAW = 'UPDATE_SAFE_WITHDRAW'; // 安全设置修改冻结出金
const COMMON_SWITCH = 'COMMON_SWITCH'; // 功能模块的开关
...
export default {
  state: {
    ...
    swiperFlag: false,
    exchief_project_switch: 1, // 是否显示全部支付方式
    exchief_arr_filter: [],
  ...
  actions: {
    ...
    getUserInfo({ commit, state }) {
      axios({
        ...
        if (data.code === '0') {
          ...
          if (state.publicInfo) {
            const publicInfoData = JSON.parse(JSON.stringify(state.publicInfo));
            if (data.data.isSub === 1) {
              publicInfoData.url.otcUrl = '';
              publicInfoData.switch.saas_otc_flow_config = 0;
              publicInfoData.switch.fe_otc_open = 0;
              commit('GETPUBLICINFO', publicInfoData);
            }
          }
    ...
    getPublicInfo({
      ...
      const sloveData = (data) => {
        ...
        axios({
          ...
          if (res.data.default_country_code || res.data.default_country_code_real) {
            commit(SET_DEFAULT_COUNTRYCODE, {
              default_country_code: res.data.default_country_code,
              default_country_code_real: res.data.default_country_code_real
                ? `+${res.data.default_country_code_real}`
                : '',
            });
          }
        });
        ...
        if (data.update_safe_withdraw && data.update_safe_withdraw.is_open === '1') {
          const hour = Number(data.update_safe_withdraw.hour);
          if (hour > 0 && hour <= 100) {
            commit(UPDATE_SAFE_WITHDRAW, hour);
          }
        }
      };
      // 获取 public-info数据
    ...
    setDefindTemplateLayoutType({ commit }, param) {
      commit('setDefindTemplateLayoutType', param);
    },
    getFunctionSwitch({ commit }) {
      axios({
        url: this.state.url.common.common_switch,
        method: 'GET',
      }).then((data) => {
        if (data.code === '0') {
          ...
          commit('COMMON_SWITCH', switchVo);
        } else {
          commit('COMMON_SWITCH', {});
          setCookie('jpSpotSwitch', '0');
        }
      });
    },
  ...
  mutations: {
    setDefindTemplateLayoutType(state, result) {
      if (result < 960) {
        state.templateLayoutType = '1';
      } else {
        state.templateLayoutType = `${state.defindTemplateLayoutType}`;
      }
    },
    ...
    [UPDATE_SAFE_WITHDRAW](state, result) {
      state.update_safe_withdraw = result;
    },
    [COMMON_SWITCH](state, result) {
      state.functionSwitch = result;
    },
  },
};
