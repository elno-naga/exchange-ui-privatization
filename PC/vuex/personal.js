...
export default {
  ...
  actions: {
    ...
    // 增加极验后修改手机号新接口
    mobileUpdateVerify({ commit }, info) {
      axios({
        url: this.state.url.common.mobile_update_verify,
        method: 'post',
        params: info,
      }).then((data) => {
        if (data.code === '0') {
    ...
    mobileBindSaveVerify({ commit }, info) {
      axios({
        url: this.state.url.common.mobile_bind_save_verify,
        method: 'post',
        params: info,
      }).then((data) => {
        if (data.code === '0') {
