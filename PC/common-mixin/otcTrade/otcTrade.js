import {
  fixD, fixInput, getCoinShowName, sendVerigicationCode,
} from '@/utils';
...
export default {
  ...
  computed: {
    ...
    isCapitalPwordSet() {
      return !!(this.userInfo && this.userInfo.isCapitalPwordSet);
    },
    googleCode() {
      return !!Number(this.userInfo.googleStatus);
    },
    smsCode() {
      return !!Number(this.userInfo.isOpenMobileCheck);
    },
    userVip() {
      let str = '';
    ...
    sendCodeType() {
      return sendVerigicationCode.c2cSell;
    },
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    verifyBtnClick() {
      const arr = [];
      if (this.isCapitalPwordSet) {
        arr.push('fundCode');
      } else if (this.enforceGoogleAuth) {
        arr.push('google');
      } else {
    ...
    BtnClick(item) {
      ...
      if (this.axiosObj.side === 'SELL') {
        ...
        if (item) {
          ...
          if (item.fundCode) {
            obj.capitalPword = item.fundCode;
          }
          if (item.google) {
    ...
    initOrder() {
      this.axios({
        ...
        if (data.code.toString() === '0') {
          ...
          if (this.rightInp && data.data.side === 'BUY') {
            this.rightServe();
          } else if (this.leftInp && data.data.side === 'SELL') {
            this.leftServe();
          }
        } else {
          this.$bus.$emit('tip', { text: data.msg, type: 'error' });
