import {
  myStorage, formatTime, imgMap, getIconPath,
} from '@/utils';
    ...
    getListData(isSearch = false) {
      ...
      this.getData({
        ...
        callback: (data) => {
          ...
          const noticeItem = data.data.noticeInfoList.find((item) => item.id === this.$route.params.ntId);

          if (!parseFloat(this.$route.params.ntId) || !noticeItem) {
