...
export default {
  ...
  methods: {
        ...
        if (data.code.toString() === '0') {
          ...
          orderList.forEach((item) => {
            ...
            list.push({
              data: item,
              id: item.id,
              time: item.time_long ? formatTime(item.time_long) : item.created_at, // 时间
              coin: `${item.baseCoin}/${quoteCoin}`, // 币对
              type: {
                text: item.side_text,
              ...
              volume: `${fixD(item.volume, coinFix)} ${item.baseCoin}`, // 数量
              amount: `${fixD(item.total_price, marketFix)} ${quoteCoin}`, // 交易额
              avgPrice: `${fixD(item.avg_price, marketFix)} ${quoteCoin}`, // 平均成交价
              percent: {
                text: `${fixD(item.deal_volume, coinFix)} ${item.baseCoin}`,
                subText: `${fixD(item.remain_volume, coinFix)} ${item.baseCoin}`,
              },
              operation: item.type.toString() === '1' ? {
                type: 'button',
        ...
        if (data.code.toString() === '0') {
          ...
          orderList.forEach((item) => {
            ...
            list.push({
              ...
              avgPrice: `${fixD(item.avg_price, marketFix)} ${getCoinShowName(quoteCoin, coinList)}`, // 平均成交价
    ...
    // 查看详情
    getSubTableData(v) {
      if (this.subContentId === v.id) {
        this.subContentId = null;
      } else {
        this.subContentId = v.id;
        const { symbol } = v;
        const symbolCoin = symbol ? `${symbol.split('/')[0]}${symbol.split('/')[1]}` : '';
        this.subContent = [];
        ...
        this.axios({
          ...
          if (data.code.toString() === '0') {
            ...
            data.data.trade_list.forEach((item) => {
              const feeCoin = item.feeCoin.toUpperCase();
              ...
              list.push({
                data: item,
                time: item.ctime,
                price: `${fixD(item.price, marketFix)} ${getCoinShowName(symbol.split('/')[1], coinList)}`,
                volume: `${fixD(item.volume, coinFix)} ${getCoinShowName(symbol.split('/')[0], coinList)}`,
                dealPrice: `${fixD(item.deal_price, marketFix)} ${getCoinShowName(symbol.split('/')[1], coinList)}`,
                fee: `${item.fee} ${getCoinShowName(feeCoin, coinList)}`,
                copmFee: item.copmFee,
                copmFeeCoin: item.copmFeeCoin,
              });
            });
            this.subLoading = false;
            this.subContent = list.map((x) => {
              const obj = x;
              if (x.copmFee && x.copmFeeCoin) {
                obj.fee = `${x.copmFee} ${x.copmFeeCoin}`;
              }
              return obj;
            });
          } else {
    ...
    cancelOrder(value) {
      ...
      if (this.revokeList.indexOf(id) === -1) {
        ...
        const symbol = value && value.symbol ? `${value.symbol.split('/')[0]}${value.symbol.split('/')[1]}` : '';
        this.axios({
          url: 'lever/order/cancel',
          method: 'post',
          params: {
            orderId: id,
            symbol: symbol ? symbol.toLowerCase() : symbol,
          },
        }).then((data) => {
