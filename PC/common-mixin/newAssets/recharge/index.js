...
export default {
  ...
  computed: {
    ...
    isSubAccountDep() {
      let bol = false;
      if (this.userInfo && this.userInfo.isSub.toString() === '1' && this.userInfo.subUserDepositOpen.toString() === '0') {
        bol = true;
      }
      return bol;
    },
  ...
  methods: {
    ...
    initAddress() {
      ...
      this.axios({
        ...
        params: {
          symbol: (this.haveBranch && this.activeBranch) ? this.activeBranch : this.symbol,
        },
      }).then((data) => {
        if (data.code.toString() === '0') {
