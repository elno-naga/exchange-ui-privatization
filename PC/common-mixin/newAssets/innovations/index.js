...
export default {
  ...
  methods: {
        ...
        if (data.code.toString() === '0') {
          ...
          if (data.data.record.length) {
            data.data.record.forEach((item) => {
              ...
              list.push({
                id: item.projectId,
                time: formatTime(item.orderTime), // 时间
                orderId: item.orderId, // 订单编号
                name: item.name, // 项目名称
                amount: `${fixD(item.investAmount, investFix)}${getCoinShowName(item.investSymbol, coinList)}`, // 申购金额
                token: `${fixD(item.tokenAmount, tokenFix)}${getCoinShowName(item.tokenSymbol, coinList)}`, // 获得代币数量
                statusText: this.setStatusText(item),
                status: item.status,
                operation: this.$t('innov.view_details'),
              });
            });
            this.tabelList = list;
    ...
    goViewPage(id) {
      this.$router.push(`/innovation/${id}`);
    },
    classesVieew(item) {
      if (item.status === 7) {
        return 'noshow';
