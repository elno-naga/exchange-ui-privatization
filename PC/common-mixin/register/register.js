import {
  getCookie, setCookie, imgMap, colorMap, getIconPath,
} from '@/utils';
...
export default {
  ...
  computed: {
    ...
    tabList() {
      // let list = [
      //   // 手机号注册
      //   { name: this.$t('register.phone2'), key: 'phone' },
      //   // 邮箱注册
      //   { name: this.$t('register.email'), key: 'email' },
      // ];
      ...
      return [
        // 邮箱注册
        { name: this.$t('register.email'), index: 'email' },
        // 手机号注册
      ...
      if (this.verificationType === '3') {
        ...
        if (
          (countryFlag
            ...
            // && comfirmFlag
            && invitedFlag
            && cms && this.verificationTypeSuccess === '3')
          || submitLoading
        ) {
          return false;
        }
    ...
    registerPageTitle() {
      ...
      return this.$t('register.welcomeToRegisterV6');
    },
    // 默认手机号
    defaultCountryCode() {
  ...
  methods: {
      ...
      if (this.nowType === 'phone') {
        ...
        this.axios({
          ...
          if (data.code.toString() === '0') {
            ...
            this.dialogConfirmLoading = false;
            this.$refs.registerInputFormulate.failedCallback(data.msg);
    ...
    setNowType({ index }) {
      ...
      if (this.countryMap) {
        const countryVal = this.countryMap[this.defaultCountryCode] || this.countryMap[`+${this.defaultCountryCode}`];
        const countryCode = countryVal ? countryVal.code : '';
      ...
      this.nowType = index;
      this.getCodeHaving = false; // 获取验证码计时结束
    ...
    submit() {
      ...
      const objData = {
        ...
        newPassword: this.passValue, // 新密码
