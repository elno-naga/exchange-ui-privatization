import {
  imgMap, colorMap, getIconPath, sendVerigicationCode,
} from '@/utils';
...
export default {
  ...
  data() {
    return {
      ...
      state: 'KYC_AUTH',
      titleText: '',
      isCancellation: '', // 是否显示 注销账号入口
      isCapitalPwordSet: 0,
      userWhiteListFlag: false,
      verifyShow: false,
      verifyDialogTitle: '',
      typeList: [],
      verifyDialogType: null,
      confirmLoading: false,
      addUpOneLevFlag: false,
      addUpInput: '',
      addUpInputErrorFlag: false,
      isSub: false,
    };
  },
  watch: {
    ...
    userInfo(userinfo) { // 监听userinfo接口
      ...
      this.userWhiteListFlag = !!userinfo.withdrawWhitelistFlag;
      this.id = userinfo.id;
    },
  ...
  computed: {
    promptText() { return this.$t('personal.dialog.name'); },
    errorText() { return this.$t('personal.dialog.text'); },
    interfaceSwitch() {
    ...
    sendCodeType() {
      let str = '';
      if (this.verifyDialogType === 'switch') {
        str = this.userWhiteListFlag
          ? sendVerigicationCode.whitelistChangeSwitchCodeClose
          : sendVerigicationCode.whitelistChangeSwitchCodeOpen;
      } if (this.verifyDialogType === 'unbind') {
        str = sendVerigicationCode.unbindFundCode;
      }
      return str;
    },
    ...
    addUpLeveShow() {
      return this.userInfo ? !!this.userInfo.ableToAddPid : false;
    },
  },
  methods: {
    init() {
    ...
    verifyDialogConfirm(item) {
      ...
      if (this.verifyDialogType === 'switch') {
        this.gridSetFormFlag(obj);
      } else if (this.verifyDialogType === 'unbind') {
        this.getUnbinding(obj);
      }
    },
    getUnbinding(obj) {
      this.confirmLoading = true;
      this.axios({
        url: 'capital_password/unbinding',
        params: obj,
      }).then((data) => {
        this.confirmLoading = false;
        if (data.code.toString() === '0') {
    ...
    gridSetFormFlag(obj) {
      this.confirmLoading = true;
      this.axios({
        url: 'withdrawWhiteListFlag',
        method: 'post',
        params: {
          ...obj,
          flag: this.userWhiteListFlag ? 0 : 1,
        },
      }).then((data) => {
        this.confirmLoading = false;
        if (data.code.toString() === '0') {
          this.userWhiteListFlag = !this.userWhiteListFlag;
          this.$store.dispatch('getUserInfo');
    ...
    getUserInfo() {
      if (this.userInfo !== null) {
        ...
        if (this.inviteUrl && this.inviteUrl.length > 40) {
          this.formattedInviteUrl = `${this.inviteUrl.substring(0, 20)}...${this.inviteUrl.substring(this.inviteUrl.length - 20)}`;
        } else {
          this.formattedInviteUrl = this.inviteUrl;
        }
        // 邀请码信息
        this.userName = this.userInfo.userAccount;
        ...
        this.isCapitalPwordSet = this.userInfo.isCapitalPwordSet;
        this.userWhiteListFlag = !!this.userInfo.withdrawWhitelistFlag;// 是否开启白名单
        ...
        this.isSub = this.userInfo.isSub.toString() !== '0';
      }
    },
    openMobile() {
    ...
    handleCancellation() {
      this.$router.push('/personal/cancellation');
    },
    changeEmail() {
      if (this.smsCode === true || this.googleCode === true) {
    ...
    getDeleteAccountStatus() {
      this.axios({
        url: 'getDeleteAccountStatus',
      }).then((data) => {
        if (data.code.toString() === '0') {
          this.isCancellation = data.data.deleteAccount;
        } else {
          this.$bus.$emit('tip', { text: data.msg, type: 'error' });
        }
      });
    ...
    // 添加上级
    addUpOneLev() {
      this.addUpOneLevFlag = true;
    },
    // 关闭添加上级
    dialogAddUpClose() {
      this.addUpInput = '';
      this.addUpInputErrorFlag = false;
      this.dialogConfirmDisabled = false;
      this.addUpOneLevFlag = false;
    },
    // 确认btn 添加上级
    dialogAddUpConfirm() {
      if (this.addUpInput && !this.addUpInputErrorFlag) {
        this.axios({
          url: 'invitation/addInvitationedCode',
          params: {
            invitedCode: this.addUpInput,
          },
        }).then((data) => {
          if (data.code.toString() === '0') {
            this.dialogAddUpClose();
            this.$store.dispatch('getUserInfo');
            this.$bus.$emit('tip', { text: this.$t('inviteFriends.invite_text34'), type: 'success' });
          } else {
            this.$bus.$emit('tip', { text: data.msg, type: 'error' });
          }
        });
      } else {
        this.addUpInputErrorFlag = true;
      }
    },
    addUpinputChanges(val) {
      this.addUpInput = val;
      if (val) {
        this.addUpInputErrorFlag = false;
      } else {
