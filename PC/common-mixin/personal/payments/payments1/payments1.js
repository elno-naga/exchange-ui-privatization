export default {
  ...
  methods: {
    ...
    uploadFinish(obj) {
      ...
      this.imgSrc = obj.url;
      this.imgFile = obj.fileName;
      const img = !Number(this.clientUploadType) ? this.imgSrc : this.imgFile;
      this.$emit('callBack', 'payment1', 'qrcodeImg', { qrcodeImg: img });
    },
    onChangeFileFun(message) {
      if (message.error) {
        this.$bus.$emit('tip', { type: 'error', text: message.error });
      } else {
