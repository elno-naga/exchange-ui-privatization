...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      // this.selectAccountList = [{ value: '全部子账户', code: 0 }];
      this.getTableList();
    },
    getSelAccountList() {
      this.axios({
      ...
      this.axios({
        ...
        if (data.code.toString() === '0') {
          this.tableList = data.data.list.map((it) => {
            ...
            if (it.authority) {
              tempObj.authority = it.authority
                .split(',')
                .filter((item) => item !== '');
            } else {
              tempObj.authority = [];
          ...
          this.tableLoading = false;
          if (!req.subUid && !req.label) this.isHaveData = data.data.list.length;
    ...
    addAPIConfirm() {
      ...
      this.axios({
        ...
        if (resp.code.toString() === '0') {
          ...
          this.currentEditRow.authority = ['0'];
    ...
    delApiConfirm() {
      this.axios({
        url: this.$store.state.url.subAccount.apiMgt_delApi,
    ...
    changeCheckBox(code, flag) {
      if (code === '0') return;
      if (flag) {
        this.currentEditRow.authority.push(code);
      } else {
        this.currentEditRow.authority = this.currentEditRow.authority.filter((it) => it !== code);
      }
    },
    // 保存API
    saveApiConfirm() {
      ...
      if (this.currentEditRow.authority.length) { this.formData.authority = this.currentEditRow.authority.join(','); }
    ...
    openDialogFn(name) {
      if (name === 'addAPIDialogFlag') {
        if (this.addSelectAccountList.length) {
          this.addAccount = this.addSelectAccountList[0].code;
        } else {
          this.addAccount = '';
        }
        this.optionType = 'add';
    ...
    sendSmsCode() {
      this.axios({
        ...
        if (data.code.toString() !== '0') {
          ...
          const successText = this.$t('login.phoneSendSuccess');
  ...
  computed: {
    ...
    columns() {
      return [
        { key: 'email', title: this.$t('subAccount.apiMgt.table_c1'), width: '220px' }, // 子账户邮箱
        { key: 'label', title: this.$t('subAccount.apiMgt.table_c2') }, // 标签
        { key: 'c', title: this.$t('subAccount.apiMgt.table_c6'), width: '60px' }, // IP限制
    ...
    ipInputErrorFlag() {
      ...
      if (this.currentEditRow.believeIps.length + ipArr.length > 10) {
        this.ipInputErrorText = this.$t('subAccount.apiMgt.eiitIp4'); // 最多只能存在5个受信任的IP地址
        return true;
      }
    ...
    openFileUrl() {
      ...
      if (lan === 'zh_CN' || lan === 'el_GR') {
        return 'https://exdocs.gitbook.io/v/v/english';
      }
      return 'https://exdocs.gitbook.io/v/general-info';
    },
    // 三方 API链接
    // openFileUrl() {
