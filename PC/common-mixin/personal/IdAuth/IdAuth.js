...
export default {
  ...
  computed: {
    ...
    promptTextType() { return this.$t('personal.idAuth.promptTextType'); },
    promptText() { return this.$t('personal.label.promptText'); },
    errorText() { return this.$t('personal.label.errorText'); },
    text1() { return this.$t('personal.label.idCard'); },
    text2() { return this.$t('personal.label.passport'); },
    promptText1() { return this.$t('personal.label.idCardNum'); },
    promptText2() { return this.$t('personal.label.name'); },
    appAuthOpen() {
  ...
  methods: {
    ...
    onChangeFileFun(obj) {
      ...
      } else if (obj.code && obj.code === 'type') {
        this.$bus.$emit('tip', { text: this.$t('personal.idAuth.uploadText')[0], type: 'error' });
      }
      this.uploadName = obj.name;
    ...
    inputChanges(value, name) {
      ...
      const regRule = /[`~^!@#$€£₤%^&*()_\-+=<>?:"{}|./;'\\[\]·~！……@#￥¥%*（）——\-+={}|《》？：“”【】；：’。，、‘']/g;
      if (name === 'name') {
        this.checkValue2 = value.replace(regRule, '');
      // 外国 - 姓
      } else if (name === 'kycFamilyName') {
        this.kycFamilyName = value.replace(regRule, '');
      // 外国 - 名
      } else if (name === 'kycName') {
        this.kycName = value.replace(regRule, '');
      // 证件号码
      } else {
        this.checkValue1 = value.replace(/[^a-zA-Z0-9]/ig, '');
      }
    },
    checkFocus(name) {
      switch (name) {
