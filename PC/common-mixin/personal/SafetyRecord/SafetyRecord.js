import { imgMap, colorMap, formatTime } from '@/utils';
  ...
  computed: {
    ...
    columns2() {
      return [
        ...
        {
          title: this.$t('personal.safetyRecord.settingColumn')[3],
          key: 'optSystem',
          align: 'left',
          width: '200px',
          classes: '',
    ...
    processData(list) {
      if (this.currentTab === 1) {
        this.dataList = list.map((obj) => (
          {
            formatLgInTime: formatTime(obj.formatLgInTimeMillis),
            lgPlatform: obj.lgPlatform,
            lgIp: obj.lgIp,
            lgStatus: (obj.lgStatus === 1 ? this.$t('personal.safetyRecord.success') : this.$t('personal.safetyRecord.failure')),
          }));
      } else {
        this.dataList2 = list.map((obj) => (
          {
            formatCtime: formatTime(obj.ctime),
            optSystem: obj.optSystem || '--',
            optTypeName: obj.optTypeName,
            optIp: obj.optIp,
          }));
      }
    },
