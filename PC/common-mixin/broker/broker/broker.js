...
export default {
  ...
  computed: {
    symbolType() {
      return this.$t('broker.symbolType');
    },
    type() {
      return this.$t('broker.type');
    },
    promptText() {
        ...
        this.tableData.forEach((item) => {
          if (this.tableType === 'agent_data') {
            dataArr.push({
              ...
              tier: item.level,
              fee: `${this.fixDFun(item.feeAmount, this.coinValue)} ${getCoinShowName(item.feeCoin, this.coinList)}`,
              amount: `${this.fixDFun(item.bonusAmount, this.coinValue)} ${getCoinShowName(item.bonusCoin, this.coinList)}`,
              operation: [
                {
                  type: 'button',
            ...
            dataArr.push({
              ...
              tier: item.level, // 层级
              number: this.fixDFun(item.amount, this.coinValue), // 持仓量
            });
          }
        });
        return dataArr;
