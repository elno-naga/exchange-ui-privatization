...
export default {
  ...
  watch: {
      ...
      if (userInfo) {
        if (userInfo.nickName) { // && userInfo.isCapitalPwordSet
          if (!userInfo.isOpenMobileCheck && !userInfo.googleStatus) {
            this.$bus.$emit('tip', { text: this.$t('modifySetting.idinfo'), type: 'info' });
          }
        } else {
          this.$bus.$emit('tip', { text: this.$t('modifySetting.idinfo'), type: 'info' });
        }
      ...
      if (publicInfo) {
        if (this.otcPublicInfo) {
          ...
          if (this.$route.query.side) {
            this.side = this.$route.query.side === 'BUY' ? 'SELL' : 'BUY';
          } else if (this.otcPublicInfo
              && this.otcPublicInfo.defaultSeach
              && this.otcPublicInfo.defaultSeach.toUpperCase() === 'BUY') {
            this.side = 'SELL';
          } else {
            this.side = 'BUY';
          }
    ...
    otcPublicInfo(otcPublicInfo) {
      if (otcPublicInfo) {
        if (this.publicInfo) {
          ...
          // defaultSeach 只负责默认选中，不负责this.side参数。 购买：SELL，出售：'BUY'
          if (this.$route.query.side) {
            this.side = this.$route.query.side === 'BUY' ? 'SELL' : 'BUY';
          } else {
            this.side = otcPublicInfo.defaultSeach.toUpperCase() === 'BUY' ? 'SELL' : 'BUY';
          }
    ...
    fiatdealALL() {
      this.list3.unshift({ value: this.$t('fiatdeal.center.all'), code: 'all' });
      this.list1.unshift({ value: this.$t('fiatdeal.center.all'), code: 'all' });
    },
  },
  computed: {
    columns() {
    ...
    authTitleText() {
      const text = 'assets.withdraw.priorityCompletion';
      return this.$t(text);
    },
    alertData() {
      const arr = [
        // 实名认证
        { text: this.$t('sumSubKyc.identityAuth'), flag: 0 },
      ];
      return arr;
    },
  },
  ...
  methods: {
    init() {
      ...
      if (userInfo !== null) {
        if (userInfo.nickName) { // && userInfo.isCapitalPwordSet
          if (!userInfo.isOpenMobileCheck && !userInfo.googleStatus) {
            this.$bus.$emit('tip', { text: this.$t('modifySetting.idinfo'), type: 'info' });// 修改
          }
        } else {
          this.$bus.$emit('tip', { text: this.$t('modifySetting.idinfo'), type: 'info' });
        }
      ...
      this.axios({
        ...
        if (code.toString() === '0') {
          if (data.c2cStatus) {
            switch (type) {
              case 'icon':
                this.$router.push({ path: '/stranger', query: { udata: id[0] } });
                break;
              default:
      ...
      if (data) {
        data.forEach((obj) => {
          list.push({
            ...
            name: `<div class="merchants">
                    <div class="merchants-left main-1-bg" style="color:#EDF4F8">
                    ${obj.imageUrl ? `<img src="${obj.imageUrl}"/>` : `<span>${obj.otcNickName.substring(0, 1)}</span>`}
                    <i class="i fill-2-bd ${obj.loginStatus === 1 ? 'rise-1-bg' : 'text-2-bg'}"></i>
                    </div>
                    <div class="merchants-right">
                    <p>${obj.otcNickName}</p>
                    <p class="text-2-cl">${this.$t('fiatdeal.center.trading')} ${obj.completeOrders} | ${this.$t('fiatdeal.center.credit')} ${nul(obj.creditGrade, 100)}%</p>
                    </div>
                    </div>`,
            volume: {
    ...
    paymentsList(data) {
      const list = [];
      data.forEach((obj) => {
        list.push(`<li class='fill-5-bg'><img src=${obj.icon} /></li>`);
      });
      return list;
    },
    ...
    dataPayments(data) {
      const List = [{ value: this.$t('mobilityTrade.all'), code: 'all' }];
      data.forEach((obj) => (
