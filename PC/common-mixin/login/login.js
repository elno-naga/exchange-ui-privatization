import {
  myStorage, setCookie, imgMap, colorMap, getCookie, getIconPath,
} from '@/utils';
...
export default {
  ...
  data() {
    return {
      ...
      loginMode: null,
      qrcodeImg:
        'https://s3.ap-northeast-1.amazonaws.com/chainup-test/qrcodeImg.png',
      qrcodeId: '', // 二维码id
      isLoading: true, // 二维码加载中
      isExpire: false, // 二维码过期
      isScaned: false, // 二维码已扫描
      isGologin: false, // 二维码去登录
      timer: null, // 二维码计时器
      hidePassword: true, // 显示/隐藏密码
      captchaObj: null, // 极验
      loginBack: window.sessionStorage.getItem('loginCallBack'), // 登陆成功回跳url,
  ...
  beforeRouteEnter(to, from, next) {
    if (from.meta && from.meta.loginBack) {
      const fromHref = window.location.origin + from.fullPath;
      window.sessionStorage.setItem('loginCallBack', window.encodeURIComponent(fromHref));
    }
    next();
  },
  beforeDestroy() {
  ...
  watch: {
    userValue(val) {
      const reg = /^[0-9]*$/;
      this.showCountryCode = val.length > 2 && reg.test(val);
    },
    checkValue(newV) {
      if (newV.length === 6) {
        this.$nextTick(() => {
  ...
  computed: {
    qrBgImg() {
      ...
      return theme.toString() === '1' ? 'https://s3.ap-northeast-1.amazonaws.com/chainup-test/login-dark-qrcode.png' : 'https://s3.ap-northeast-1.amazonaws.com/chainup-test/login-light-qrcode.png';
    },
    // 默认手机号
    defaultCountryCode() {
    ...
    // passValue 是否复合正则验证
    passFlag() {
      // this.$store.state.regExp.passWord.test(this.passValue);
      return this.passValue.length;
    },
    // checkValue 是否复合正则验证
    checkFlag() {
    ...
    submitDisabled() {
      const {
        ...
        userFlag, passFlag, submitLoading,
      } = this;
      // if ((userFlag && passFlag && verifyFlag) || submitLoading) {
      if (this.verificationType === '3') {
        if ((userFlag && passFlag && this.verificationTypeSuccess === '3') || submitLoading) {
          return false;
        }
        return true;
      } if (this.verificationType === '2') {
        if ((userFlag && passFlag) || submitLoading) {
          return false;
        }
        return true;
      }
    ...
    cmpQR_Login() {
      if (this.publicInfo && this.publicInfo.switch) {
        return this.publicInfo.switch.QR_Login;
      }
      return '0';
    },
    companyName() {
  ...
  methods: {
    init() {
      ...
      // 加载页面的时候加载二维码
      if (this.cmpQR_Login === '1') this.refresh();
    },
    setStyle() {
      if (colorMap) {
        let str;
    ...
    goPage() {
      ...
      if (this.loginBack) {
        window.location.href = window.decodeURIComponent(this.loginBack);
        window.sessionStorage.removeItem('loginCallBack');
        // } else if (this.jumpFlag) {
        //   this.$router.push('/loginResult');
        // } else {
        //   this.$router.push('/v5/assets/exchangeAccount');
        // }
      } else if (returnUrl) {
    ...
    inputChanges(value, name) {
      // 解决2次
      if (this.verifyObj && this.verifyObj.token !== true) {
        this[name] = value;
      }
      if (name === 'checkValue') {
    ...
    submit() {
      ...
      if (this.showCountryCode) {
        params.countryCode = this.countryKeyCode;
      }
      this.submitLoading = true;
      this.axios({
        url: 'v6/user/login_in',
        headers: {},
        params,
        method: 'post',
      }).then((data) => {
    ...
    dialogConfirm() {
      ...
      this.axios({
        ...
        if (data.code.toString() === '0') {
          ...
          this.dialogConfirmLoading = false;
          this.$refs.loginInputFormulate.failedCallback(data.msg);
          // this.$bus.$emit('tip', { text: data.msg, type: 'error' });
        }
      });
    },
      ...
      this.axios({
        ...
        if (code.toString() === '0') {
          ...
          if (status.toString() === '1') {
            ...
            this.getMySymbol();
            // this.$router.push('/');
            if (this.typeAlertFlag) {
              window.sessionStorage.security = 1;
            }
            let lang = '';
            if (this.$store.state.baseData.lan) {
              lang = this.$store.state.baseData.lan;
            }
