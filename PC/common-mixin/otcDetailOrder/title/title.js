    ...
    domArr() {
      ...
      if (this.isReady) {
        ...
        if (status.toString() === '1' && side === 'BUY') {
          arr.push(
            // 待付款
            { text: this.$t('otcDetailOrder.bar1Buy1'), class: 'fill-5-bg text-1-cl' },
            // 待收币
            { text: this.$t('otcDetailOrder.bar1Buy2'), class: 'fill-5-bg text-1-cl' },
          );
        // 已拍下 -- 待收款 -- 待放币
        } else if (status.toString() === '1' && side === 'SELL') {
          arr.push(
            // 待收款
            { text: this.$t('otcDetailOrder.bar1Sell1'), class: 'fill-5-bg text-1-cl' },
            // 待放币
            { text: this.$t('otcDetailOrder.bar1Sell2'), class: 'fill-5-bg text-1-cl' },
          );
        // 已拍下 -- 已付款 -- 待收币
        } else if (status.toString() === '2' && side === 'BUY') {
          arr.push(
            // 已付款
            { text: this.$t('otcDetailOrder.bar2Buy1'), class: 'main-1-bg text-1-cl' },
            // 待收币
            { text: this.$t('otcDetailOrder.bar2Buy2'), class: 'fill-5-bg text-1-cl' },
          );
        // 已拍下 -- 已收款 -- 待放币
        } else if (status.toString() === '2' && side === 'SELL') {
          arr.push(
            // 已收款
            { text: this.$t('otcDetailOrder.bar2Sell1'), class: 'main-1-bg text-1-cl' },
            // 待放币
            { text: this.$t('otcDetailOrder.bar2Sell2'), class: 'fill-5-bg text-1-cl' },
          );
        // 已拍下 -- 已付款 -- 已完成
        } else if (status.toString() === '3' && side === 'BUY') {
          arr.push(
            // 已付款
            { text: this.$t('otcDetailOrder.bar3Buy1'), class: 'main-1-bg text-1-cl' },
            // 已完成
            { text: this.$t('otcDetailOrder.bar3Buy2'), class: 'main-1-bg text-1-cl' },
          );
        // 已拍下 -- 已收款 -- 已完成
        } else if (status.toString() === '3' && side === 'SELL') {
          arr.push(
            // 已收款
            { text: this.$t('otcDetailOrder.bar3Sell1'), class: 'main-1-bg text-1-cl' },
            // 已完成
            { text: this.$t('otcDetailOrder.bar3Sell2'), class: 'main-1-bg text-1-cl' },
          );
        // 已拍下 -- 已关闭
        } else if (status.toString() === '4' || status.toString() === '9') {
          arr.push(
            // 已关闭
            { text: this.$t('otcDetailOrder.bar4Sell'), class: 'main-1-bg text-1-cl' },
          );
        // 已拍下 -- 申诉中
        } else if (status.toString() === '5') {
          arr.push(
            { text: this.$t('otcDetailOrder.bar5Sell'), class: 'main-1-bg text-1-cl' },
          );
        // 已拍下 -- 异常订单
        } else if (status.toString() === '7') {
          arr.push(
            { text: this.$t('otcDetailOrder.bar7Sell'), class: 'main-1-bg text-1-cl' },
          );
        // 已拍下 -- 已付款 -- 已完成
        } else if (status.toString() === '8' && side === 'BUY') {
          arr.push(
            // 已付款
            { text: this.$t('otcDetailOrder.bar8Buy1'), class: 'main-1-bg text-1-cl' },
            // 已完成
            { text: this.$t('otcDetailOrder.bar8Buy2'), class: 'main-1-bg text-1-cl' },
          );
        // 已拍下 -- 已收款 -- 已完成
        } else if (status.toString() === '8' && side === 'SELL') {
          arr.push(
            // 已收款
            { text: this.$t('otcDetailOrder.bar8Sell1'), class: 'main-1-bg text-1-cl' },
            // 已完成
            { text: this.$t('otcDetailOrder.bar8Sell2'), class: 'main-1-bg text-1-cl' },
          );
        }
      }
      return arr;
