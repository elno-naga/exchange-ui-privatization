import {
  fixD, colorMap, imgMap, getIconPath,
} from '@/utils';
  ...
  computed: {
    ...
    currentSymbol() {
      ...
      if (this.projectDetail && this.projectDetail.gainCoin && this.symbolAll) {
        const { showPrecision } = this.symbolAll[this.projectDetail.gainCoin] || { showPrecision: 8 };
        fix = showPrecision;
      }
    ...
    columns() {
      return [
        { title: this.$t('freeStaking.detail.incomeTime'), key: 'time' }, // 币种
        { title: this.$t('freeStaking.detail.incomeNum'), key: 'amount' }, // 地址
      ];
    },
    // 二次确认按钮是否开启
    confirmBtnFlag() {
  ...
  methods: {
    ...
    alertGo() {
      if (this.alertData && this.alertData.length) {
        this.$router.push(this.alertData[0].url);
      } else {
    ...
    submitUserPos() {
      const {
        buyAmountMin, buyAmountMax, balance, shortName, totalAmount, needAuth,
      } = this.projectDetail;
      let errText = '';
      ...
      //   // arr.push(
      //   //   { text: this.$t('assets.withdraw.bindPhone'), flag: false, url: '/personal/userManagement' },
      //   // );
      //   this.showToast(errText);
      //   return;
      // }
      ...
      // arr.push(
      //   {
      //     text: this.$t('assets.withdraw.bindGoogle'), flag: false, url: '/personal/userManagement', btnText: this.$t('otcRelease.ToSetUp'),
      //   },
      // );
      // this.showToast(errText);
      // return;
      ...
      if (this.userInfo.authLevel === 0 && needAuth === 1) {
        arr.push(
          {
            text: this.$t('sumSubKyc.identityAuth'), flag: false, url: '/personal/identityAuthen', btnText: this.$t('sumSubKyc.goToVerify'),
          },
        );
      }
      if (arr.length > 0) {
