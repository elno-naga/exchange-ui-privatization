...
export default {
  data() {
    return {
      ...
      projectList: [],
      homeData: {
        banner: imgMap.freestaking_banner,
      },
      imgMap,
      colorMap,
      iconStyle: {
  ...
  computed: {
    ...
    navTab() {
      if (!this.homeData || !this.homeData.detail) return [];
  ...
  methods: {
    ...
    countStatusIsOn(item) {
      ...
      return lock || unLock ? 'main-1-cl' : 'text-2-cl';
    },
    ...
    currentStyle(item) {
      return item.index === this.currentStatus ? ['text-1-cl', 'fill-6-bg'] : ['text-2-cl'];
    },
