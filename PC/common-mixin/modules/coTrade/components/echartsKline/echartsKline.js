...
export default {
  ...
  computed: {
    ...
    pricefix() {
      if (this.acriveContract) {
        if (this.acriveContract.priceFix === 0) {
          return 0;
        }
  ...
  methods: {
    ...
    setData(obj) {
      return {
        time: obj.mtimeTimestamp ? obj.mtimeTimestamp : new Date(obj.mtime).getTime(),
        close: obj.close,
        open: obj.open,
        high: obj.high,
        low: obj.low,
            ...
            if (code === '0') {
              ...
              if (data.records && data.records.length) {
                ...
                if (arr.length) {
                  ...
                  let seconds = 0;
                  if (this.beginTime) {
                    seconds = new Date(this.beginTime).getSeconds();
                  }
                  if (seconds) {
                    this.beginTime -= seconds * 1000;
                  }

                  this.lastTime = reverseArr[reverseArr.length - 1].time;
                  if (isreq) {
                    if (reverseArr[reverseArr.length - 1]) { callbackFun(reverseArr); }
