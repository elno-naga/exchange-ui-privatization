...
export default {
  ...
  data() {
    return {
      ...
      // 杠杆输入框选中Class
      inputActiveClass: '',
      isOnMouseDown: false, // 是否正在滑动
      positionType: 2, // 1全仓 2逐仓
    };
  },
  props: {
    isShow: {
  ...
  computed: {
    lanText() {
      return {
        ...
        text71: this.$t('futures.tradeForm.text71'), // 计算结果仅供参考
      };
    },
    // 用户配置信息
    userConfig() {
      ...
      return [
        {
          ...
          name: this.lanText.text16, // '收益',
          classes: this.categoryType === 1 ? 'main-1-bg' : 'fill-6-bg',
        },
        {
          id: 2,
          text: this.lanText.text13, // '平仓价格',
          name: this.lanText.text13, // '平仓价格',
          classes: this.categoryType === 2 ? 'main-1-bg' : 'fill-6-bg',
        },
        {
          id: 3,
          text: this.lanText.text11, // '强平价格',
          name: this.lanText.text11, // '强平价格',
          classes: this.categoryType === 3 ? 'main-1-bg' : 'fill-6-bg',
        },
      ];
    },
    // 类型（收益，平仓价格， 强平价格）
    sideTypeList() {
      ...
      //       classes: this.sideType === 1 ? 'text-1-cl' : 'text-1-cl main-1-f-h',
      //     },
      //     {
      //       id: 2,
      //       text: this.lanText.text18, // '空仓',
      //       classes: this.sideType === 2 ? 'text-1-cl' : 'text-1-cl main-1-f-h',
      //     },
      //   ];
      // }
      return [
        {
          ...
          classes: this.sideType === 1 ? 'text-4-cl' : 'text-1-cl main-1-f-h',
        },
        {
          id: 2,
          text: this.lanText.text38, // '卖出开空',
          classes: this.sideType === 2 ? 'text-4-cl' : 'text-1-cl main-1-f-h',
        },
      ];
    },
    // 杠杆选项
    ...
    positionTypeList() {
      return [
        { code: 1, value: this.$t('futures.positionLis.text14') }, // 全仓
        { code: 2, value: this.$t('futures.positionLis.text15') }, // 逐仓
      ];
    },
    // 是否登录
    isLogin() {
    ...
    canUseAmount() {
      if (
        ...
        return fixD(this.accountBalance.canUseAmount, this.minOrderMoneyFix);
    ...
    positionType(val) {
      if (this.categoryType === 3) {
        ...
        if (val === 1) {
          ...
          if (this.isLogin) {
            // 强平价格->可用余额
            this.formData_3.value = this.thousandsComma(this.canUseAmount);
          } else {
  ...
  methods: {
    ...
    formVerify() {
      if (this.categoryType === 3) {
        if (this.positionType === 1 && this.formData_1.value && this.formData_2.value && this.formData_3.value) {
          return true;
        } if (this.positionType === 2 && this.formData_1.value && this.formData_2.value) {
          return true;
        }
        return false;
      }
    ...
    submit() {
      ...
      if (this.categoryType === 1) {
        ...
        const v1 = this.contractSide === 1
          ? nul(volume, openPrice)
          : division(volume, openPrice);
        ...
        if (Number(this.openMargin) > 0) {
          this.openMarginClass = 'rise-1-cl';
        }
        if (Number(this.openMargin) < 0) {
          this.openMarginClass = 'fall-1-cl';
        }
        // 收益额
        // 正向合约：
        ...
        if (Number(this.revenue) > 0) {
          this.revenueClass = 'rise-1-cl';
        }
        if (Number(this.revenue) < 0) {
          this.revenueClass = 'fall-1-cl';
        }
        // this.contractSide === 1
      }
      ...
      if (this.categoryType === 2) {
        ...
        // 杠杆 - 回报率
        if (
          (this.contractSide === 1 && this.sideType === 2)
        ...
        if (value <= 0) {
          ...
          if (Number(this.closePrice) > 0) {
            this.closePriceClass = 'rise-1-cl';
          }
          if (Number(this.closePrice) < 0) {
            this.closePriceClass = 'fall-1-cl';
          }
        }
      }
      // 强平价格全仓
      if (this.categoryType === 3 && this.positionType === 1) {
        // 强平价格
        // （单位：计价货币）
        // 正向合约：
        ...
        if (this.coUnitType === 2) {
          // 张
          Y1 = closePrice * openPrice; // (仓位数量 * 开仓价格)
        } else {
          Y1 = this.formData_2.value * openPrice; // (仓位数量 * 开仓价格)
        }
        ...
        const volume2 = this.contractSide === 1 ? division(nul(closePrice, openPrice), this.leverage) : division(closePrice, nul(openPrice, this.leverage)); // 开仓保证金
        // 多仓：强平价格>开仓价格 时 不显示强平价格，显示：可用余额不足以开仓！
        if (this.sideType === 1 && value > openPrice) {
          this.forceClosePrice = this.lanText.text23; // '可用余额不足以开仓';
          this.forceClosePriceNUm = false;
          this.forceClosePriceClass = 'fall-1-cl';
        } else if (this.sideType === 2 && value < openPrice) {
          this.forceClosePrice = this.lanText.text23; // '可用余额不足以开仓';
          this.forceClosePriceNUm = false;
          this.forceClosePriceClass = 'fall-1-cl';
        } else if (volume < volume2) {
          // 当可用余额<开仓保证金时，提示“可用余额不足以开仓”
          this.forceClosePrice = this.lanText.text23; // '可用余额不足以开仓';
          this.forceClosePriceNUm = false;
          this.forceClosePriceClass = 'fall-1-cl';
        } else {
          this.forceClosePrice = this.thousandsComma(
            fixD(value, this.pricefix),
          );
          this.forceClosePriceNUm = true;
          if (Number(value) > 0) {
            this.forceClosePriceClass = 'rise-1-cl';
          }
      ...
      // 强平价格逐仓
      if (this.categoryType === 3 && this.positionType === 2) {
        // ===================
        // 强平价格
        // （单位：计价货币）
        // 正向合约：
        // 开仓保证金 = 开仓价格*仓位数量/杠杆
        // 多头：强平价格 = （仓位数量 * 开仓价格 - 开仓保证金 / 保证金汇率 ） / （（1 - 维持保证金率 - 手续费率）* 仓位数量）
        // 空头：强平价格 = （仓位数量 * 开仓价格 + 开仓保证金 / 保证金汇率 ） / （（1 + 维持保证金率 + 手续费率）* 仓位数量)

        // 反向合约：
        // 开仓保证金 = 仓位数量/(开仓价格*杠杆倍数)
        // 多头：强平价格 = （1 + 维持保证金率 + 手续费率）* 仓位数量 /（（仓位数量/开仓价格）+ 开仓保证金/保证金汇率）
        // 空头：强平价格 = （1 - 维持保证金率 - 手续费率）* 仓位数量 /（（仓位数量/开仓价格）- 开仓保证金/保证金汇率）
        ...
        // ===========
        closePrice = this.setNumber(this.formData_2.value); // 3:仓位数量
        volume = this.contractSide === 1 ? division(nul(closePrice, openPrice), this.leverage) : division(closePrice, nul(openPrice, this.leverage)); // 开仓保证金
        const sRate = 0.00075; // 手续费
        let Y1;
        if (this.coUnitType === 2) {
          // 张
        ...
        const mRate = this.getladderValue(Y1); // 获取维持保证金率
        const Y2 = division(volume, this.marginRate); // 开仓保证金 / 保证金汇率
        // 正向 = 仓位数量 * 开仓价格
        // 反向 = 仓位数量 / 开仓价格
        const Y3 = this.contractSide === 1
          ? nul(closePrice, openPrice)
          : division(closePrice, openPrice);
        let Y4 = cut(Y3, Y2); // 正向 多仓 || 反向 空仓 = 仓位数量 * 开仓价格-开仓保证金 / 保证金汇率 :
        let Y5 = cut(cut(1, mRate), sRate); // 正向 多仓 || 反向 空仓 = （1-维持保证金率-手续费率）
        //  正向 空仓 || 反向 多仓
        if (
          (this.contractSide === 1 && this.sideType === 2)
          || (this.contractSide === 0 && this.sideType === 1)
        ) {
          // 仓位数量 * 开仓价格 + 开仓保证金 / 保证金汇率
          Y4 = add(Y3, Y2);
          // 1 + 维持保证金率 + 手续费率
          Y5 = add(add(1, mRate), sRate);
        }
    ...
    categoryTypeSwitch(item) {
      if (this.categoryType !== item.id) {
    ...
    setMaxNumber() {
      ...
      if (this.leverCeiling) {
        ...
        for (let index = 0; index < keyArr.length; index += 1) {
          if (
            Number(this.leverage) > nextL
            && Number(this.leverage) <= keyArr[index]
          ) {
    ...
    onmousedownCick(e) {
      if (!this.isOnMouseDown) {
        ...
        const stepPosition = parseInt(
          nul(division(leftVal, bar.offsetWidth), 100),
          0,
        );
    ...
    onmousedown(event) {
      ...
      document.onmousemove = (e) => {
        ...
        const stepPosition = parseInt(
          nul(division(movement, bar.offsetWidth), 100),
          0,
        );
        this.stepPosition = stepPosition < 1 ? 1 : stepPosition;
        this.currenttage = stepPosition < 1 ? 1 : stepPosition;
      ...
      if (this.ladderList && this.ladderList.length) {
        ...
        if (value === 0) {
          ...
          this.ladderList.forEach((item) => {
            if (
              value > item.minPositionValue
              && value <= item.maxPositionValue
            ) {
    ...
    // 全仓逐仓切换
    positionTypeChange(item) {
      if (this.orderType === item.code) { return; }
      this.positionType = item.code;
    },
  },
};
