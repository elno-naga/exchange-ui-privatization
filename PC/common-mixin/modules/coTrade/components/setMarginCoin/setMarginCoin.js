// 调节逐仓仓位保证金
import {
  nul, add, cut, division, fixD, fixInput, thousandsComma, getIconPath,
} from '@/utils';
...
export default {
  ...
  computed: {
      ...
      return [
        {
          ...
          classes: this.marginReviseType === 1 ? 'text-1-cl' : 'text-1-cl main-1-f-h',
        },
        {
          id: 2,
          text: this.lanText.text11, // '减少保证金',
          classes: this.marginReviseType === 2 ? 'text-1-cl' : 'text-1-cl main-1-f-h',
        },
      ];
    },
    // 增加&减少保证金 提示文案
    ...
    // 修改后的强平价格
    modifiedReducePrice() {
      if (!this.marginValue || this.marginValue === '0') return this.marginData.reducePrice;
  ...
  methods: {
    ...
    setPercentageVlaue(value) {
      ...
      const amount = this.marginReviseType === 1
        ? this.marginData.canUseAmount : this.marginData.canSubMarginAmount;
      const number = division(amount, 100 / value);
      this.marginValue = number;
    },
    // 鼠标移入
