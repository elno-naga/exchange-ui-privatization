import {
  fixD, nul, division, thousandsComma, getIconPath,
} from '@/utils';
...
export default {
          ...
          dataTypeKey.forEach((item) => {
            ...
            objKeys.forEach((itemKey) => {
              ...
              if (dataArr.length < 30) {
                ...
                // 盘口精度切换
                this.activeDepatinfo = this.depathList[this.activeDepat];
                const objd = {
                  // 总量
                  total,
                  // 价格
                  price: fixD(itemArr[0], this.activeDepatinfo),
                  // 数量
                  vol,
                  // 是否有变化
                  diff: itemArr[2],
  ...
  watch: {
    ...
    isShowDepat(val) {
      if (val) {
        this.childSelectWidth = this.$refs.childSelect.offsetWidth;
      }
    },
  },
  methods: {
    ...
    switchDepthType(type) {
      switch (type) {
        ...
        default:
          ...
          this.sellLineNumber = 9;
          this.buyLineNumber = 9;
      }
    },
    // 新价精度
    fixPrice(value) {
