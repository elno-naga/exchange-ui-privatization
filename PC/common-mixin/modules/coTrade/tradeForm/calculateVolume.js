...
export default {
  ...
  computed: {
    ...
    maxBuyNumber() {
      ...
      if (this.contractSide === 1 && this.coUnitType === 2) {
        ...
        return division(nul(this.V1Buy, this.marginRate), nul(this.priceVal('BUY'), this.multiplier));
      }
      // 反向 & 张数
      if (this.contractSide === 0 && this.coUnitType === 2) {
        // V1Buy * 价格 / 面值
        return division(nul(this.V1Buy, this.priceVal('BUY')), this.multiplier);
      }
      // 正向 & 量
      if (this.contractSide === 1 && this.coUnitType === 1) {
        // V1Buy * 保证金汇率 / 价格
        return division(nul(this.V1Buy, this.marginRate), this.priceVal('BUY'));
      }
      // 反向 & 量
      if (this.contractSide === 0 && this.coUnitType === 1) {
        // V1Buy * 价格
        return nul(this.V1Buy, this.priceVal('BUY'));
      }
      return 0;
    },
    // 最大可卖
    maxSellNumber() {
      ...
      if (this.contractSide === 1 && this.coUnitType === 2) {
        ...
        return division(nul(this.V1Sell, this.marginRate), nul(this.priceVal('SELL'), this.multiplier));
      }
      // 反向 & 张数
      if (this.contractSide === 0 && this.coUnitType === 2) {
        // V1Sell * 价格 / 面值
        return division(nul(this.V1Sell, this.priceVal('SELL')), this.multiplier);
      }
      // 正向 & 量
      if (this.contractSide === 1 && this.coUnitType === 1) {
        // V1Sell * 保证金汇率 / 价格
        return division(nul(this.V1Sell, this.marginRate), this.priceVal('SELL'));
      }
      // 反向 & 量
      if (this.contractSide === 0 && this.coUnitType === 1) {
        // V1Sell  价格
        return nul(this.V1Sell, this.priceVal('SELL'));
      }
      return 0;
    },
    // 买入可平数量
    ...
    maxCanBuyNumber() {
      ...
      const number = this.calculateMaxNumber('BUY') < this.maxBuyNumber
        ? this.calculateMaxNumber('BUY') : this.maxBuyNumber;
    ...
    maxCanSellNumber() {
      ...
      const number = this.calculateMaxNumber('SELL') < this.maxSellNumber
        ? this.calculateMaxNumber('SELL') : this.maxSellNumber;
      if (number < 0) return 0;
      return fixD(number, this.volfix);
    },
  ...
  methods: {
    ...
    calculateMaxNumber(type) {
      if (!this.canUseAmount || !this.priceVal(type)) return 0;
      // 正向
      if (this.contractSide === 1) {
        ...
        if (this.coUnitType === 2) {
          // 价格 * 合约面值
          const v2 = nul(this.priceVal(type), this.multiplier);
          return division(V1, v2);
        }
        // 量 （可用余额 *杠杆/汇率）/价格
        return division(V1, this.priceVal(type));
      }
      // 反向
      // 量
      // 可用余额 * 杠杆* 价格
      const V3 = nul(nul(this.canUseAmount, this.nowLevel), this.priceVal(type));
    ...
    priceVal(type) {
      ...
      // 限价
      const price = this.isRivalPrice ? this.rivalPrice[type] : this.inputPrice;
      return price ? Number(price) : price;
    },
    calculateCostNumber(type) {
      let volume = this.inputVolume || 0;
      if (this.percentageVlaue) {
        volume = this.countPercentageVlaue(type) || 0;
      }
      let number = 0;
      // 平仓 || 只减仓 （仓位价值）
      ...
      if (this.currentCategory === 1 || (this.currentCategory === 3 && !this.isMarket)) {
        ...
        number = nul(
          division(
            division(nul(volume, this.multiplier), price),
            this.nowLevel,
          ), this.marginRate,
        );
        ...
        if (this.contractSide === 1) {
          number = nul(
            division(
              nul(
                nul(volume, this.multiplier), price,
              ),
              this.nowLevel,
            ),
            this.marginRate,
