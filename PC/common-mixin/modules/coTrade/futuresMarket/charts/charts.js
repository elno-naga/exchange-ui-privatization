  ...
  methods: {
    init() {
      ...
      if (this.name) {
        ...
        let currentTime = new Date().getTime() - 20000;
        for (let index = 0; index < 21; index += 1) {
          const val = this.random(1, 100);
          currentTime += 1000;
          defaultDataList.push([currentTime, val]);
        }
        this.echartsData = defaultDataList;
        this.eachart();
        this.$forceUpdate();
      }
    },
        ...
        bg = {
          normal: {
            color: new window.echarts.graphic.LinearGradient(
              0, 0, 0, 1,
              [
                { offset: 0, color: 'rgba(239,185,11,0.2)' },
                { offset: 1, color: 'rgba(239,185,11,0.05)' },
              ],
            ),
          },
        };
