import {
  fixInput, nul, division, fixD, thousandsComma, getIconPath,
} from '@/utils';
...
export default {
  ...
  computed: {
    ...
    activeContractName() {
      ...
      if (this.contractInfo) {
        const nameText = this.contractInfo.contractOtherName
          ? this.contractInfo.contractOtherName : '';
        name = `${nameText}`;
      }
      return name;
    },
    ...
    // 最大可选范围
    userMaxWidth() {
      return (this.userMaxLevel / this.maxLeverage) * 100;
    },
  ...
  watch: {
    ...
    leverage(val, old) {
      if (val) {
        if (Number(val) > this.userMaxLevel && this.userMaxLevel) {
  ...
  methods: {
    ...
    subtract() {
      if (this.levelSwitch) {
        this.isClickAddOrSub = true;
        if (Number(this.leverage) > this.minLeverage) {
          const leverage = Number(this.leverage) - 1;
          this.leverage = leverage;
          this.isShowUserMinLevel = false;
        } else {
          this.leverage = this.minLeverage;
          this.isShowUserMinLevel = true;
        }
      }
    },
    // 提交杠杆倍数
    ...
    dragStepItemClass(index) {
      const classIN = `drag-step${index}`;
      let classes = 'fill-3-bg';
      if (index * this.itemLvaue <= this.userMaxLevel) {
        classes = 'fill-3-bg';
      }
      if (index * 20 <= this.stepPosition) {
        classes = 'main-1-bg';
      }
      return [classIN, classes];
    },
