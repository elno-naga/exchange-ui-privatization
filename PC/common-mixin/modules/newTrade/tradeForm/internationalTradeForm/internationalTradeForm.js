import {
  myStorage, nul, getCoinShowName, fixRate, fixD, imgMap, getIconPath,
} from '@/utils';
    ...
    buttosContent() {
      return {
        buyButton: {
          ...
          class: this.isLogin
            ? 'rise-1-bg buyBtn buttonStyleColor'
            : 'main-1-bg buyBtn text-4-cl',
        },
        sellButton: {
          text: this.isLogin
            ...
            : this.$t('trade.loginReg'), // '登录/注册',
          class: this.isLogin
            ? 'fall-1-bg sellBtn buttonStyleColor'
            : 'main-1-bg sellBtn text-4-cl',
        },
      };
    },
    // 账户资产
        ...
        if (formKey.length) {
          formKey.forEach((item) => {
            ...
            if (parseFloat(value) <= 0) {
              this[item].errorText = `${this[item].title}${this.$t(
                'trade.inputError',
              )}`; // 输入有误
              this[item].isError = true;
              fal = false;
