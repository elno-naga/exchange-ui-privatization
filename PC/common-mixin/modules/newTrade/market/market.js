import {
  getCoinShowName, myStorage, getIconPath, imgMap,
} from '@/utils';
...
export default {
  ...
  computed: {
    ...
    marketOptions() {
      const list = this.marketList.slice(3);
      return list.map((item) => ({
        value: item,
        label: getCoinShowName(item, this.coinList),
      }));
    },
    isLogin() {
    ...
    coinDataList() {
      const list = [];
      Object.values(this.marketDataList).forEach((item) => {
        list.push(item);
      });
      return list;
    },
    proTrade() {
      ...
      return activeName === 'proTrade' || activeName === 'proTradeMargin';
    },
    showMarketList() {
      if (this.marketList.length > 4) {
        return this.marketList.slice(0, 3);
      }
  ...
  watch: {
    ...
    listfilterVal(val) {
      ...
      if (val) {
        ...
        this.dataList = this.coinDataList.filter((item) => {
          if (item.showSymbol) {
  ...
  methods: {
    init() {
      ...
      this.$bus.$on('showLeftMarket', () => {
        this.shrinkBlock();
      });
      setTimeout(() => {
        let step = this.marketList.indexOf(this.marketCurrent);
        if (step > 2) {
          step = 3;
          this.marketSelect = this.marketCurrent;
        }
        this.setTabStyle(step + 1);
      }, 1000);
      ...
      this.$bus.$on('MARKET_DATA', (data) => {
        this.marketDataList = JSON.parse(JSON.stringify(data));
        this.setData();
      });
      this.$bus.$on('isMobile', (data) => {
        this.isMobile = data;
      });
      this.$bus.$on('proTradeMarketIsShow', () => {
        this.shrinkBlock();
      });
      this.$bus.$on('order_switchSymbol', (symbol) => {
        this.switchSymbol(symbol);
        const market = symbol.split('/')[1];
        let index = this.marketList.indexOf(market) + 1;
        index = index > 4 ? 4 : index;
        this.switchMarket(market, index);
      });
    },
    symbolTitle(data) {
    ...
    setTabStyle(index) {
      this.$nextTick(() => {
        ...
        const { offsetLeft, offsetWidth } = Array.isArray(tab) ? this.$refs[`tab-${index}`][0] : tab;
    ...
    // 切换市场
    switchMarket(data, index) {
      if (this.listfilterVal) return;
    ...
    marketChange(item) {
      this.switchMarket(item.value, 4);
      this.marketSelect = item.value;
    },
    getFocus() {
