...
export default {
  ...
  methods: {
    ...
    futuresLink(link, target) {
      ...
      if (link === '/order/coOrder') {
        this.$router.push('/order/coOrder');
        return;
      }
      if (link === '/assets/coAccount') {
        this.$router.push('/assets/coAccount');
        return;
      }
  ...
  computed: {
    ...
    tradesList() {
      ...
      if (this.headerLink.otc) {
        if (this.publicInfo && this.publicInfo.switch.credit_crad_switch === '1') {
          // 同时开启P2P交易和信用卡交易
          arr.push({
            title: this.fiatTradeOpen ? this.$t('assets.b2c.otcShow.header') : this.$t('header.otc'),
            activeId: 'otcTrade',
            children: [{
              title: this.$t('assets.b2c.otcShow.fiatdealTitle'),
              activeId: 'otcTrade',
              link: this.headerLink.otc,
            },
            {
              title: this.$t('creditCardPurchase.title'),
              activeId: 'creditCardPurchase',
              link: `${this.headerLink.credit}`,
            },
            ],
          });
        } else {
      ...
      } else if (this.headerLink.otc === '') {
        if (this.publicInfo && this.publicInfo.switch.credit_crad_switch === '1') {
          // 只开启信用卡入金
          arr.push({
            title: this.$t('assets.b2c.otcShow.header'),
            activeId: 'creditCardPurchase',
            link: `${this.headerLink.credit}`,
          });
        } else {
          // P2P交易和信用卡交易都不开启：导航栏隐藏“买币”菜单；什么都不加入
        }
      }
    ...
    headerLink() {
      if (process.env.NODE_ENV === 'development') {
        return {
          ...
          credit: '/ex/zh_CN/creditCardPurchase',
        };
      }
      if (this.$store.state.baseData && this.$store.state.baseData.publicInfo) {
        return {
          ...
          credit: this.linkurl.exUrl ? `${this.linkurl.exUrl}/${this.lan}/creditCardPurchase` : '',
        };
      }
      return '';
    },
      ...
      if (this.userInfo && this.userInfo.accountStatus.toString()) {
        switch (this.userInfo.accountStatus.toString()) {
          ...
          case '2':
            ...
            break;
          case '3':
            str = this.$t('header.userStatus4'); // '冻结交易 3冻结提现';
            break;
          default:
            break;
        }
