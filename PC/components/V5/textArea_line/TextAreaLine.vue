...
<template>
  <section class='area-line-content' :style='contentStyle' :class='className'>
    <!-- 基础占位 46px 包含下划线 -->
    <div
      class='input-line-baseStance fill-5-bd'
    ...
    >
      ...
      >
        ...
        <textarea
          :maxlength='maxLength'
          class='common-textarea text-1-cl'
          :type='nowType'
          required='true'
          v-model='curValue'
          :disabled='disabled'
    ...
    <div class='input-line-errorStence' v-if='errorHave'>
      <!-- 错误信息容器 -->
      <p class='input-line-error fall-1-cl' v-if='errorFlag && !isFocus'>
        {{ errorText }}
      </p>
      <!-- 警示文案 -->
  ...
  watch: {
    curValue(v) {
      if (!v) {
        ...
        this.$refs.area_line_inp.style.height = `${30}px`;
      }
    },
  },
  methods: {
