...
<template>
  <div>
    <section class='common-geetest' :style='contentStyle'  v-if="product!== 'bind'">
      <!-- 基础部分 占位42px -->
      <div
        class='common-geetest-baseStance fill-5-bd'
      ...
      >
        ...
        <div id='captchaBox' class='fill-2-bg' ref='captchaBox'></div>
        <!-- 选中时横线 -->
        <span
      ...
      <!-- 错误文案占位 24px -->
      <div class='common-geetest-errorStence fall-1-cl' v-if='errorHave'>
        <!-- 错误信息容器 -->
        <p class='common-geetest-error' v-if='errorFlag'>{{ errorText }}</p>
      </div>
    </section>
  <div id='captchaBox' class='fill-2-bg' ref='captchaBox' v-if="product=== 'bind'"></div>
  </div>
</template>
<script>
export default {
    ...
    setStyle() {
      ...
      if (colorMap) {
        if (radar && radar.length) {
          radar[0].style.color = colorMap['text-2-cl'];
        }
        if (reset && reset.length) {
          reset[0].style.color = colorMap['text-2-cl'];
        }
      ...
      window.initGeetest(
        ...
        (captchaObj) => {
          ...
          captchaObj.onError((err) => {
            console.error('极验出错了', err);
            this.$bus.$emit('tip', { text: this.$t('common.errorTipText'), type: 'error' });
          });
