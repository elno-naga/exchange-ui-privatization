  ...
  <div class="common-navTab" :class="navTabClass">
    ...
    >
      <li
        ...
        :style="contentStyle"
        :ref="'tabItem' + item.index"
        :class="color(item)"
        :key="index"
        @mouseover="hoverIndex = item.index"
        @mouseleave="hoverIndex = null"
        @click="currentClick(item)"
      >
        <template v-if="type === 'iconBlock' && item.icon">
          <span v-if="item.index !== currentTab" v-html="item.icon"></span>
...
export default {
  ...
  computed: {
    ...
    showActiveBlock() {
      const activeItem = this.navTab.find((item) => item.index === this.currentTab);
  ...
  mounted() {
    ...
    if (this.navTab.length && this.navTab[0].name) {
      this.getNodeDetail();
    }
  },
  methods: {
    ...
    color(item) {
      ...
      // hover状态
      if (this.hoverIndex === item.index && item.index !== this.currentTab) {
        if (this.type === 'line') {
          return this.activeClassName || 'text-1-cl';
        }
        if (this.type === 'block') {
      ...
      } else if (this.hoverIndex === item.index && this.type === 'card') {
    ...
    setActiveLine(index) {
      ...
      if (tabItem && tabItem.length) {
        ...
        const childLeft = tabItem[0].offsetLeft;
        const itemWidth = tabItem[0].offsetWidth;
        // 底部线性动画
        if (this.type === 'line') {
          const left = childLeft - parentLeft + (itemWidth - this.activeLineWidth) / 2;
